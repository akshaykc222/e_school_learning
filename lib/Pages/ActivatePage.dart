import 'package:e_school_learning/Constants.dart';
import 'package:e_school_learning/Pages/LoginPageWeb.dart';
import 'package:e_school_learning/Pages/SchoolPage.dart';
import 'package:e_school_learning/Provider/LoginProvider.dart';
...
class _ActivatePageState extends State<ActivatePage> with SingleTickerProviderStateMixin{
  ...
  _checkSession() async {
    ...
    if(prefs.getBool('activated')!=null){
      Navigator.pushNamed(context, routeLoginWeb);
    }
...
showBottomDialogSchool(){
            ...
            builder: (_,provider,child){
              return Container(
                ...
                child: Column(
                  ...
                  children: [
                    Text("Welcome",style: TextStyle(color: Colors.black,fontWeight: FontWeight.bold,fontSize: 25,decorationThickness: 5),),
                    SizedBox(height: 20,),
                    Image.network(provider.imageUrl),
                    SizedBox(height: 20,),
                    Padding(
                      padding: const EdgeInsets.all(8.0),
                      child: Text(provider.schoolName,style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: Colors.black,overflow: TextOverflow.fade),),
                    ),
                    SizedBox(height: 30,),
                    Padding(
                      padding: const EdgeInsets.all(8.0),
  ...
  void sizeAnimationOnTap(){
    sizeAnimationController=AnimationController(vsync: this,duration: Duration(milliseconds: 400));
...
void _onTap() {//-->function for ripple animation transition
  ...
  void _goToNextPage() {//-->function for navigation
                    ...
                    child: InkWell(
                      onTap: () async {
                        ...
                        if (_formKey.currentState!.validate()) {
                         ...
                         int statusCode=await Provider.of<LoginProvider>(context,listen: false).getActivation(secretKeyController.text);
                         if(statusCode==200){
                         setState(() {
                           isLoading=false;
                           _showSchool=true;
                           ...
                           final snackBar = SnackBar(content: Text(' Invalid activation key.'));
                      ...
                      child: AnimatedBuilder(
                        ...
                        builder: (BuildContext context, Widget? child){
                          print('${(width*0.4)+sizeAnimation.value} size animation');
                          return Padding(
                          padding: const EdgeInsets.all(8.0),
                          child: Container(
                            width: (width*0.4-20)+sizeAnimation.value,
                            height: 40+sizeAnimation.value,
                            decoration: BoxDecoration(
                              borderRadius: BorderRadius.circular(10),
                              image: DecorationImage(
                                image: AssetImage(activationButton),
                                fit: BoxFit.fill
                              )
                            ),
                          ),
